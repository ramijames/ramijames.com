...
const fragmentShader = `
  ...
  void main() {
    ...
    float shimmer = sin(vPosition.x * 8.0 + uTime * uAnimationSpeed * 0.3) * 
    ...
    brightness = clamp((brightness - 0.5) * 1.4 + 0.5 + uThreshold, 0.0, 1.0);
    ...
    float grain = random(vUv + uTime * 0.05) * 0.03;
    dithered = clamp(dithered + grain, 0.0, 1.0);
    
    // Invert for dark mode if needed
    if (uInvert > 0.5) {
...
onMounted(async () => {
  ...
  geometry = new THREE.PlaneGeometry(90, 90, 200, 200)
